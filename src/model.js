...
import { roofPoint, roofTileSectors, tileElevation, hipTileBeddingGeometry, ROOF_COLOR_SCALE } from './roof-geometry.js';
...
export function buildPavilion({ plaqueMap, surfaceImages } = {}) {
  ...
  function buildRoof(inner, outer, base, height, ridgeCount) {
    ...
    for (let sector = 0; sector < 6; sector++) {
      ...
      for (let j = 0; j < hipPieces; j++) {
        ...
        cap.updateMatrix();
        mesh(hipTileBeddingGeometry(cap.geometry, cap.matrix, sector, { inner, outer, base, height }), mats.tileDark).name = '垂脊·随形承托';
      }
  ...
  root.userData.design = { type: 'original-artistic-pavilion', units: 'metres', roofDeckThickness: .055, pavingRadius: 3.74, revision: 13 };
